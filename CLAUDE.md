...
│   ├── assets/SpriteGen.ts  # Canvas-drawn spritesheets → PNG data URLs (Jesus walk, holy-beam, holy-water)
...
- `mountMobileControls` calls `resetMobileInput()` on mount so module state doesn't bleed between runs (e.g. an active joystick on run N doesn't phantom-move the player on run N+1).
